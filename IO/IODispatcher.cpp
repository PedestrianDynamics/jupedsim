...
void TrajectoriesJPSV04::WriteHeader(
    long nPeds,
    double fps,
    Building * building,
    int seed,
    ...
    for(const auto & it : building->GetAllRooms()) {
        ...
        for(auto && sitr : r->GetAllSubRooms()) {
            ...
            if(plotHlines) {
                ...
                // the transitions
                if(plotTransitions) {
                    const std::vector<Transition *> & transitions = s->GetAllTransitions();
...
void TrajectoriesXML_MESH::WriteSources(
    const std::vector<std::shared_ptr<AgentsSource>> /*sources*/)
{
}
