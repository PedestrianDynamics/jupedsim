...
JPSserver::JPSserver(
    HybridSimulationManager * hsm,
    std::shared_ptr<Latches> latches,
    Configuration * configuration) :
    _HybridSimulationManager(hsm),
    _latches(latches),
...
grpc::Status JPSserver::simulatedTimeInerval(
...
::grpc::Status JPSserver::transferAgent(
    ::grpc::ServerContext * context,
    ...
    ::hybridsim::Boolean * response)
{
    ...
    for(const hybridsim::Link & l : request->leg().link()) {
        ...
        for(auto && rm :
            bld->GetAllRooms()) { //TODO: store rooms in a more adequate data structure to avoid linear search (e.g. quadtree)
    ...
    for(auto && src : srcs) //TODO: linear search is inefficient, replace it by a map
    {
        ...
        if(enterC.x() > bounds[0] && enterC.x() < bounds[1] && enterC.y() > bounds[2] &&
           enterC.y() < bounds[3]) {
                ...
                for(auto && goal : goals) { //TODO: replace linear search by smthng more efficient
                    if(goal.second->Contains(leaveP)) {
                        ...
                        if(_configuration->GetDumpScenario()) {
                            _HybridSimulationManager->GetSimObserver()->AddPedestrian(
                                src, goal.second->GetId());
                        }
                        break;
                    }
                }
...
::grpc::Status JPSserver::receiveTrajectories(
    ::grpc::ServerContext * context,
...
::grpc::Status JPSserver::retrieveAgents(
    ::grpc::ServerContext * context,
    const ::hybridsim::Empty * request,
...
::grpc::Status JPSserver::shutdown(
    ::grpc::ServerContext * context,
    const ::hybridsim::Empty * request,
    ::hybridsim::Empty * response)
{
...
::grpc::Status JPSserver::initScenario(
    ::grpc::ServerContext * context,
