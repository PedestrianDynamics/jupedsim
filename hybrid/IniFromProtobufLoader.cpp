...
void IniFromProtobufLoader::configureGompertz(const hybridsim::Model & model)
{
    ...
    std::shared_ptr<DirectionStrategy> exit_strategy =
        configureExitCrossingStrategy(model.gompertz().exit_crossing_strategy());
    ...
    _configuration->SetModel(std::shared_ptr<OperationalModel>(new GompertzModel(
        exit_strategy,
        _configuration->GetNuPed(),
        _configuration->GetaPed(),
        _configuration->GetbPed(),
        _configuration->GetcPed(),
        _configuration->GetNuWall(),
        _configuration->GetaWall(),
        _configuration->GetbWall(),
...
IniFromProtobufLoader::configureExitCrossingStrategy(google::protobuf::int32 strategy)
{
    std::shared_ptr<DirectionStrategy> exit_strategy;
...
void IniFromProtobufLoader::setForcePed(const hybridsim::Force & force)
{
    ...
    Log->Write(
        "INFO:\tfrep_ped mu=%0.2f, a=%0.2f, b=%0.2f c=%0.2f",
        _configuration->GetNuPed(),
        _configuration->GetaPed(),
        _configuration->GetbPed(),
        _configuration->GetcPed());
...
void IniFromProtobufLoader::setForceWall(const hybridsim::Force & force)
{
    ...
    Log->Write(
        "INFO:\tfrep_wall mu=%0.2f, a=%0.2f, b=%0.2f c=%0.2f",
        _configuration->GetNuWall(),
        _configuration->GetaWall(),
        _configuration->GetbWall(),
        _configuration->GetcWall());
...
void IniFromProtobufLoader::configureAgentParams(const hybridsim::AgentParams & params)
{
    ...
    auto agentParameters =
        std::shared_ptr<AgentsParameters>(new AgentsParameters(para_id, _configuration->GetSeed()));
...
void IniFromProtobufLoader::IniRoutingStrategies(
    const google::protobuf::RepeatedPtrField<hybridsim::Router> & field)
{
