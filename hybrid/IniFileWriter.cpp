...
void IniFileWriter::WriteTrajectoryOutputDefinition()
{
    ...
    str << "\t<trajectories format=\"" << fm << "\" fps=\"" << _configuration->GetFps() << "\">"
        << std::endl;
    if(!_configuration->GetTrajectoriesFile().empty()) {
        str << "\t\t<file location=\"" << _configuration->GetTrajectoriesFile() << "\"/>"
            << std::endl;
    } else {
        str << "\t\t<!-- <file location=\"somelocation.xml\"/> -->" << std::endl;
    }
    if(_configuration->GetHostname() != "" && _configuration->GetPort() != 0) {
        str << "\t\t<socket hostname=\"" << _configuration->GetHostname() << "\" port=\""
            << _configuration->GetPort() << "\"/>" << std::endl;
    } else {
...
void IniFileWriter::WriteGoals(std::stringstream & str)
{
    for(auto && entry : _simulation->GetBuilding()->GetAllGoals()) {
        ...
        str << "\t\t\t<goal id=\"" << goal->GetId() << "\" final=\""
            << (goal->GetIsFinalGoal() == 0 ? "false" : "true") << "\" caption=\""
            << goal->GetCaption() << "\">" << std::endl;
        const std::vector<Point> & p = goal->GetPolygon();
        str << "\t\t\t\t<polygon>" << std::endl;
        for(Point pnt : p) {
            str << "\t\t\t\t\t<vertex px=\"" << pnt._x << "\" py=\"" << pnt._y << "\" />"
                << std::endl;
        }
        str << "\t\t\t\t\t<vertex px=\"" << p.begin()->_x << "\" py=\"" << p.begin()->_y << "\" />"
            << std::endl;
        str << "\t\t\t\t</polygon>" << std::endl;
        str << "\t\t\t</goal>" << std::endl;
    }
...
void IniFileWriter::WriteAgents()
{
    ...
    str << "\t<agents operational_model_id=\""
        << "2"
    ...
    for(auto entry : _simObserver->GetAllRelations()) {
        ...
        str << "\t\t\t<group group_id=\"" << ++grpIds << "\" room_id=\"" << sd->GetRoomId()
            << "\" "
               "subroom_id=\""
            << sd->GetSubroomID() << "\" number=\"" << od.nrAgents << "\" goal_id=\"" << od.goalId
            << "\" router_id=\"" << sd->GetRouterId() << "\" agent_parameter_id=\"1\" x_min=\""
            << bounds[0] << "\" x_max=\"" << bounds[1] << "\" y_min=\"" << bounds[2]
            << "\" y_max=\"" << bounds[3] << "\" />" << std::endl;

        std::stringstream src;
        src << "\t\t\t<source id=\"" << ++srcIds
            << "\" frequency=\"1\" agents_max=\"0\" group_id=\"" << grpIds << "\" caption=\"source "
            << srcIds << "\"/>" << std::endl;
...
void IniFileWriter::WriteOpModels()
{
    ...
    if(GompertzModel * g = dynamic_cast<GompertzModel *>(
           op)) { //TODO bad practice and poor oop design [gl Apr. '16]
        str << "\t\t<model operational_model_id=\"2\" description=\"gompertz\">" << std::endl;
        WriteGompertzModel(str);
    } else {
        std::cerr << "operational model:" << op->GetDescription()
                  << " not yet supported by IniFileWriter" << std::endl;
        exit(-1);
    }
    ...
    for(Router * r : _configuration->GetRoutingEngine().get()->GetAvailableRouters()) {
        switch(r->GetStrategy()) {
            case RoutingStrategy::ROUTING_GLOBAL_SHORTEST:
                str << "\t\t<router router_id=\"" << r->GetID()
                    << "\" description=\"global_shortest\">" << std::endl;
                str << "\t\t\t<parameters>" << std::endl;
                str << "\t\t\t</parameters>" << std::endl;
                str << "\t\t</router>" << std::endl;
                break;
            default:
                std::cerr << "Routing strategy: " << r->GetID()
                          << " not yet supported by IniFileWriter" << std::endl;
                exit(-1);
        }
    }
...
void IniFileWriter::WriteGompertzModel(std::stringstream & str)
{
    ...
    switch(_configuration
               ->GetSolver()) { //TODO bad practice, add at least enum in Macros.h [gl Apr. '16]
        case 1:
    ...
    str << "\t\t\t\t<exit_crossing_strategy>"
        << "3"
        << "</exit_crossing_strategy> <!-- currently, ``3'' is hard coded "
           "as the IniFileWriter does not support anything else [gl Apr. '16] -->"
        << std::endl;
    str << "\t\t\t\t<linkedcells enabled=\""
        << (_configuration->GetLinkedCellSize() > 0. ? "true" : "false") << "\" cell_size=\""
        << _configuration->GetLinkedCellSize() << "\" />" << std::endl;
    str << "\t\t\t\t<force_ped nu=\"" << m->GetNuPed() << "\" b=\"" << m->GetbPed() << "\" c=\""
        << m->GetcPed() << "\" />" << std::endl;
    str << "\t\t\t\t<force_wall nu=\"" << m->GetNuWall() << "\" b=\"" << m->GetbWall() << "\" c=\""
        << m->GetcWall() << "\" />" << std::endl;
    str << "\t\t\t</model_parameters>" << std::endl;
}
...
void IniFileWriter::WriteAgentParameters(std::stringstream & str)
{
    ...
    for(auto entry : _configuration->GetAgentsParameters()) {
        ...
        str << "\t\t\t<!-- IniFileWriter cannot reconstruct parameters' distributions, setting "
               "sigma to 0.0 for all "
               "entries. Change them to something else at your convenience -->"
            << std::endl;
