...
data_1_filename = 'reference_data_Method_1_I_IFD.dat'
data_1 = np.loadtxt(data_1_filename, usecols = (0,1,2,3,4,5,6))

data_2_filename = './Output_Methods_1/Fundamental_Diagram/IndividualFD/IFD_I_traj_Methods.txt_id_1.dat'
...
if (Frame_exit == True) and (PersID_exit == True) and (x_exit == True) and (y_exit == True) and (z_exit == True) and (velocity_exit == True) and (density_exit == True):
    logging.info('Method I: IFD-Test passed successfully.')
    logging.shutdown()
    sys.exit(0)
else:
    logging.info('Method I: IFD-Test passed not successfully.')
    logging.shutdown()
    sys.exit(1)
