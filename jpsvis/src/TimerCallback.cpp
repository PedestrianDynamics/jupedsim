...
void TimerCallback::Execute(vtkObject * caller, unsigned long eventId, void * callData)
{
    ...
    if(extern_first_dataset_loaded) {
        ...
        if(extern_is_pause) {
            frame = extern_trajectories_firstSet.getFrame(
                extern_trajectories_firstSet.getFrameCursor());
        } else {
    ...
    sprintf(
        ...
        nPeds,
        frameNumber * iren->GetTimerDuration(tid) / 1000);
...
void TimerCallback::updateSettings(vtkRenderWindow * renderWindow)
{
    ...
    extern_glyphs_pedestrians_actor_2D->SetVisibility(
        SystemSettings::getShowAgents() && SystemSettings::get2D());
    extern_glyphs_pedestrians_actor_3D->SetVisibility(
        SystemSettings::getShowAgents() && !SystemSettings::get2D());
    extern_glyphs_directions_actor->SetVisibility(
        SystemSettings::getShowDirections() && SystemSettings::get2D());
...
void TimerCallback::takeScreenshotSequence(vtkRenderWindow * renderWindow)
{
    ...
    screenshots.append(
        ...
        SystemSettings::getFilenamePrefix());
