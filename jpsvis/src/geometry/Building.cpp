...
Room* Building::GetRoom(int index) const
{
    ...
    if(_rooms.count(index) == 0) {
        Log::Error(
            "Wrong 'index' in CBuiling::GetRoom() Room ID: %d size: %d", index, _rooms.size());
...
bool Building::LoadGeometry(const std::string& geometryfile)
{
    ...
    if(!docGeo.LoadFile()) {
        ...
        Log::Error(
            "LoadGeometry: could not parse the geometry file %s", geoFilenameWithPath.c_str());
        return false;
    }
...
void Building::AddTransition(Transition* line)
{
    if(_transitions.count(line->GetID()) != 0) {
        Log::Error(
            "Duplicate index for transition found [%d] in Routing::AddTransition()", line->GetID());
        exit(EXIT_FAILURE);
    }
...
Transition* Building::GetTransition(int ID)
{
    if(_transitions.count(ID) == 1) {
        ...
        if(ID == -1)
            ...
            Log::Error(
                "I could not find any transition with the 'ID' [%d]. You have defined [%d] "
                "transitions",
                ID,
                _transitions.size());
            exit(EXIT_FAILURE);
