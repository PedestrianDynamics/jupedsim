...
bool SubRoom::AddWall(const Wall & w)
{
    ...
    for(auto && w1 : _walls) {
        if(w == w1) {
            Log::Error(
                "Duplicate wall found in Room/Subroom %d/%d  %s",
                _roomID,
                _id,
                w.toString().c_str());
    ...
    if(w.Length() < J_TOLERANZ) {
        Log::Error(
            "Wall too small (length = %lf) found in Room/Subroom %d/%d  %s",
            w.Length(),
            _roomID,
            _id,
            w.toString().c_str());
...
bool SubRoom::IsVisible(const Line & wall, const Point & position)
{
    ...
    // printf(" \t  Wall (%f, %f)--(%f, %f)\n",wall.GetPoint1().GetX(),
    // wall.GetPoint1().GetY(),wall.GetPoint2().GetX(), wall.GetPoint2().GetY() );

    bool wall_is_vis = true;
    // Point nearest_point =  wall.ShortestPoint(position);
    // in cases where nearest_point is endPoint of Wall, the wall becomes visible even if it is
    // not..
    ...
    for(auto & w : _walls) {
        ...
        if(wall_is_vis && ped_wall.IntersectionWith(w)) {
            // fprintf (stdout, "\t\t Wall_is_visible: INTERSECTION WALL  L1_P1(%.2f, %.2f),
            // L1_P2(%.2f, %.2f), WALL(%.2f, %.2f)---(%.2f, %.2f)\n",
            // ped_wall.GetPoint1().GetX(),ped_wall.GetPoint1().GetY(), ped_wall.GetPoint2().GetX(),
            // ped_wall.GetPoint2().GetY(),
    ...
    for(const auto & obst : _obstacles) {
        for(const auto & w : obst->GetAllWalls()) {
            if(wall_is_vis && ped_wall.IntersectionWith(w)) {
                // fprintf (stdout, "\t\t Wall_is_visible INTERSECTION OBS; L1_P1(%.2f, %.2f),
                // L1_P2(%.2f, %.2f), L2_P1(%.2f, %.2f) L2_P2(%.2f, %.2f)\n", w.GetPoint1().GetX(),
                // w.GetPoint1().GetY(), w.GetPoint2().GetX(), w.GetPoint2().GetY(),
                // ped_wall.GetPoint1().GetX(), ped_wall.GetPoint1().GetY(),
                // ped_wall.GetPoint2().GetX(), ped_wall.GetPoint2().GetY());
                wall_is_vis = false;
            }
        }
    }
    ...
    for(const auto & wall : _walls) {
        for(const auto & obst : _obstacles) {
            if(obst->IntersectWithLine(wall)) {
                Log::Error(
                    "the obstacle id [%d] is intersection with subroom [%d]", obst->GetId(), _id);
...
bool SubRoom::SanityCheck()
{
    ...
    for(auto && w1 : _walls) {
        ...
        for(auto && w2 : _walls) {
            ...
            if(w1.Overlapp(w2)) {
                Log::Error(
                    "Overlapping between walls %s and %s ",
                    w1.toString().c_str(),
                    w2.toString().c_str());
                exit(EXIT_FAILURE);
                // return false;
        ...
        for(auto && hline : _hlines) {
            if(w1.Overlapp(*hline)) {
                Log::Error(
                    "Overlapping between wall %s and  Hline %s ",
                    w1.toString().c_str(),
        ...
        for(auto && c : _crossings) {
            if(w1.Overlapp(*c)) {
                Log::Error(
                    "Overlapping between wall %s and  crossing %s ",
                    w1.toString().c_str(),
                    c->toString().c_str());
                exit(EXIT_FAILURE);
            }
        ...
        for(auto && t : _transitions) {
            if(w1.Overlapp(*t)) {
                Log::Error(
                    "Overlapping between wall %s and  transition %s ",
                    w1.toString().c_str(),
                    t->toString().c_str());
                exit(EXIT_FAILURE);
            }
        ...
        if(!connected) {
            Log::Error(
                "loose wall found %s  in Room/Subroom %d/%d", w1.toString().c_str(), _roomID, _id);
            exit(EXIT_FAILURE);
        }
    }
    ...
    for(auto && h : _hlines) {
        for(auto && c : _crossings) {
            if(h->Overlapp(*c)) {
                Log::Error(
                    "Overlapping between hline %s and  crossing %s ",
                    h->toString().c_str(),
                    c->toString().c_str());
                exit(EXIT_FAILURE);
            }
    ...
    for(auto && h : _hlines) {
        for(auto && t : _transitions) {
            if(h->Overlapp(*t)) {
                Log::Error(
                    "Overlapping between hline %s and  transition %s ",
                    h->toString().c_str(),
                    t->toString().c_str());
                exit(EXIT_FAILURE);
            }
    ...
    for(auto && c : _crossings) {
        for(auto && t : _transitions) {
            if(c->Overlapp(*t)) {
                Log::Error(
                    "Overlapping between crossing %s and  transition %s ",
                    c->toString().c_str(),
                    t->toString().c_str());
                exit(EXIT_FAILURE);
            }
...
bool NormalSubRoom::ConvertLineToPoly(const vector<Line *> & goals)
{
    ...
    if((tmpPoly[0] - point).Norm() > J_TOLERANZ) {
        Log::Error(
            "NormalSubRoom::ConvertLineToPoly(): SubRoom %d Room %d Anfangspunkt ungleich "
            "Endpunkt!!!\n"
            "\t(%f, %f) != (%f, %f)\n",
            GetSubRoomID(),
            GetRoomID(),
...
bool Stair::ConvertLineToPoly(const vector<Line *> & goals)
{
    ...
    if((*aktPoint - *firstOtherPoint).Norm() < J_TOLERANZ) {
        ...
        y2 = aktPoint->GetY();
        Log::Error(
            "Stair::ConvertLineToPoly(): SubRoom %d Room %d Anfangspunkt ungleich Endpunkt!!!\n"
            "\t(%f, %f) != (%f, %f)\n",
            GetSubRoomID(),
            GetRoomID(),
            x1,
    ...
    if(orgPoly.size() != 4) {
        Log::Error(
            "Stair::ConvertLineToPoly(): Stair %d Room %d ist kein Viereck!!!\n"
            "Anzahl Ecken: %d\n",
            GetSubRoomID(),
            (int) GetRoomID(),
            (int) orgPoly.size());
