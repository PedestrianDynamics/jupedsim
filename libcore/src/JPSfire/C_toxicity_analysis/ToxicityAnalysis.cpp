...
double ToxicityAnalysis::CalculateFEDIn(
    Pedestrian * p,
    double CO2,
    double CO,
    double O2,
...
void ToxicityAnalysis::HazardAnalysis(Pedestrian * p)
{
    ...
    WriteOutHazardAnalysis(
        p, E, FEC_Smoke, O2, CO2, CO, HCN, HCL, FED_In, FIC_Im, FIC_In, T, FED_Heat);
}
...
void ToxicityAnalysis::WriteOutHazardAnalysis(
    const Pedestrian * p,
    double E,
    double FEC_Smoke,
    double O2,
    ...
    double FED_Heat)
{
    ...
    sprintf(
        tmp,
        "\t<agent "
        "ID=\"%i\"\tt=\"%.0f\"\tE=\"%.4f\"\tFEC_Smoke=\"%.4f\"\tc_O2=\"%.0f\"\tc_CO2=\"%.0f\"\tc_"
