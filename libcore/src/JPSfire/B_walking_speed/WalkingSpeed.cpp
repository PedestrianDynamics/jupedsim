...
bool WalkingSpeed::LoadJPSfireInfo(const std::string & projectFilename)
{
    ...
    if(JPSfireCompElem) {
        if(JPSfireCompElem->FirstAttribute()) {
            ...
            std::string extinction_grids =
                xmltoa(JPSfireCompElem->Attribute("extinction_grids"), "");
...
double WalkingSpeed::Jin1978(double & walking_speed, double ExtinctionCoefficient)
{
    ...
    if(irritant == "false") {
        walking_speed = std::fmax(
            0.3,
            walking_speed * (-0.54991616 * std::pow(ExtinctionCoefficient, 3) +
...
double WalkingSpeed::Fridolf2018(double & walking_speed, double ExtinctionCoefficient)
{
    ...
    walking_speed =
        std::fmin(1, std::fmax(0.2, (1.0 - 0.34 * (3.0 - (2.0 / ExtinctionCoefficient)))));
    return walking_speed;
}
