...
ParseTrainTimeTableNode(TiXmlElement* node, const std::map<std::string, TrainType>& trainTypes)
{
    ...
    return TrainEventInfo{id, trackID, trainTypes.at(type), trainOffset, reversed};
}
            ...
            if(trainTimes.has_value()) {
                ...
                result.events.push_back(
                    {std::chrono::duration_cast<std::chrono::nanoseconds>(
                         std::chrono::duration<double>(arrivalTime)),
                     TrainEvent::Type::ARRIVAL,
                     trainType,
                     trainInfo->trainStartOffset,
                     trainInfo->trainID,
                     trainInfo->trackID,
                     trainInfo->reversed});
                ...
                result.events.push_back(
                    {std::chrono::duration_cast<std::chrono::nanoseconds>(
                         std::chrono::duration<double>(departureTime)),
                ...
                result.trains.emplace(trainInfo->trainID, trainType);
            }
        }
    }
    return result;
