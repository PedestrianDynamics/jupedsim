...
private:
    ...
    double _edgeCost;
    int _exitsCnt;
    // if false, the router will only return the exits and not the navigations line created through
    // the mesh or inserted via the routing file. The mesh will only be used for computing the
    // distance.
