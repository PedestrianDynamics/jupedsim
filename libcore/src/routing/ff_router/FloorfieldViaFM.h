...
public:
    ...
    bool operator<(const TrialP & rhs) const { return this->cost[this->key] < rhs.cost[rhs.key]; }
...
public:
    ...
    void checkNeighborsAndAddToNarrowband(
        std::priority_queue<TrialP, std::vector<TrialP>, std::greater<TrialP>> & trialfield,
        TrialP key,
