...
void SubRoom::AddGoalID(int ID)
{
    ...
    if(std::find(_goalIDs.begin(), _goalIDs.end(), ID) != _goalIDs.end()) {
        ...
        // if occurs, plz assert, that ID is a UID of any line of the goal and not a number given by
        // the user (ar.graf)
    }
...
bool NormalSubRoom::ConvertLineToPoly(const std::vector<Line*>& goals)
{
    ...
    for(auto& it : _walls) {
        ...
        ++itr; // only check lines that have greater index than current "it" (inner loop goes from
               // itr to size)
    }
...
// we ignore
//@todo: ar.graf: UnivFF have subroomPtr Info for every gridpoint. Info should be used in
// DirectionFF instead of this
