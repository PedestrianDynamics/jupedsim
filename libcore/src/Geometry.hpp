...
public:
    ...
    using DoorRange        = IteratorPair<DistanceQueryIterator<Door>>;
    /// Do not call constructor drectly use 'GeometryBuilder'
    /// @param segments line segments constituting the geometry
    explicit Geometry(std::vector<Line> && segments, std::vector<Door> && doors);
    /// Default destructor
    ~Geometry() = default;
    /// Non-copyable
    ...
    /// Returns an iterator pair to all doors <= 'distance' away from 'p'
    /// @param distance from reference point
    /// @param p reference point
    /// @return iterator_pair to all doors in range
    DoorRange DoorsInDistanceTo(double distance, Point p) const;
    /// Will perfrom a linesegment intersection versus the whole geometry, i.e. walls and closed doors.
    ...
    /// maipulate state of door with specific id.
    /// @param id of door to modify
    /// @param newState for door
    void UpdateDoorState(int id, DoorState newState);

    /// The following methods are temporay until we have completely migrated from building to geometry
    /// and function to support the required geometry modification for trains in the mean time.
