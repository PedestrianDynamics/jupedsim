...
.. py:class:: AgentStep(obj: jupedsim.native.AgentView)

   Bases: :py:obj:`AgentView`
   ...
   .. py:method:: with_neighbor_state_mapping(repack: Callable[[Any], Any]) -> AgentStep
      ...
      :param repack: Callable ``(neighbor_state) -> state``, called once per
                     neighbor per query. It has to return a built-in model state.

      :returns: A new :class:`AgentStep`. Only valid during the current callback.
...
.. py:class:: AgentView(obj: jupedsim.native.AgentView)
   ...
   .. py:method:: other_agents_in_range(radius: float, predicate: Callable[[NeighborView], bool] | None = None) -> list[NeighborView]

      Return agents within *radius*, excluding the agent itself.

      :param radius: Search radius.
      :param predicate: Optional callable ``(neighbor) -> bool``. Only neighbors
      ...
      :returns: List of :class:`NeighborView`. Only valid during the current callback.



   .. py:method:: walls_in_range(distance: float) -> list[WallView]
   ...
   .. py:method:: with_neighbor_state_mapping(repack: Callable[[Any], Any]) -> AgentView
      ...
      :returns: A new :class:`AgentView`. Only valid during the current callback.
...
.. py:class:: CustomOperationalModel
   ...
   .. py:method:: check_model_constraint(state: Any, view: jupedsim.agent_view.AgentView) -> None

      Raise an exception when *state* violates this model's constraints.

      :param state: The model state to validate.
      :param view: What the agent perceives of its surroundings.



   .. py:method:: compute_next_state(state: Any, step: jupedsim.agent_view.AgentStep) -> tuple[Any, tuple[float, float]]
      :abstractmethod:


      Compute one update for the agent this model is called for.

      :param state: The agent's current model state, the object you handed to
                    :meth:`~jupedsim.simulation.Simulation.add_agent`.
      :param step: What the agent perceives during this step, relative to where
                   it stands. Only valid for the duration of this call.

      :returns: A pair ``(new_state, movement)``. ``new_state`` must be a new
...
.. py:class:: WallView(obj: jupedsim.native.WallView)
   ...
   .. py:property:: segment
      :type: jupedsim.linesegment.LineSegment
