...
#include "jupedsim/collision_free_speed_model_v3.h"
...
double JPS_CollisionFreeSpeedModelV3State_GetRangeXScale(JPS_CollisionFreeSpeedModelV3State handle)
{
    assert(handle);
    const auto state = reinterpret_cast<const CollisionFreeSpeedModelV3Data*>(handle);
    return state->rangeXScale;
}

void JPS_CollisionFreeSpeedModelV3State_SetRangeXScale(
    JPS_CollisionFreeSpeedModelV3State handle,
    double rangeXScale)
{
    assert(handle);
    auto state = reinterpret_cast<CollisionFreeSpeedModelV3Data*>(handle);
    state->rangeXScale = rangeXScale;
}

double JPS_CollisionFreeSpeedModelV3State_GetRangeYScale(JPS_CollisionFreeSpeedModelV3State handle)
{
    assert(handle);
    const auto state = reinterpret_cast<const CollisionFreeSpeedModelV3Data*>(handle);
    return state->rangeYScale;
}

void JPS_CollisionFreeSpeedModelV3State_SetRangeYScale(
    JPS_CollisionFreeSpeedModelV3State handle,
    double rangeYScale)
{
    assert(handle);
    auto state = reinterpret_cast<CollisionFreeSpeedModelV3Data*>(handle);
    state->rangeYScale = rangeYScale;
}
...
JPS_CollisionFreeSpeedModelV3State_GetThetaMaxUpperBound(JPS_CollisionFreeSpeedModelV3State handle)
{
    assert(handle);
    const auto state = reinterpret_cast<const CollisionFreeSpeedModelV3Data*>(handle);
    return state->thetaMaxUpperBound;
}

void JPS_CollisionFreeSpeedModelV3State_SetThetaMaxUpperBound(
    JPS_CollisionFreeSpeedModelV3State handle,
    double thetaMaxUpperBound)
{
    assert(handle);
    auto state = reinterpret_cast<CollisionFreeSpeedModelV3Data*>(handle);
    state->thetaMaxUpperBound = thetaMaxUpperBound;
}

double JPS_CollisionFreeSpeedModelV3State_GetAgentBuffer(JPS_CollisionFreeSpeedModelV3State handle)
{
    assert(handle);
    const auto state = reinterpret_cast<const CollisionFreeSpeedModelV3Data*>(handle);
    return state->agentBuffer;
}

void JPS_CollisionFreeSpeedModelV3State_SetAgentBuffer(
    JPS_CollisionFreeSpeedModelV3State handle,
    double agentBuffer)
{
    assert(handle);
    auto state = reinterpret_cast<CollisionFreeSpeedModelV3Data*>(handle);
    state->agentBuffer = agentBuffer;
}
