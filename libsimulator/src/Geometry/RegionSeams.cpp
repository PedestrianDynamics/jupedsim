...
std::vector<RegionSeam> extract_region_seams(const SurfaceMesh& mesh, const RegionMap& region)
{
    ...
    for(const auto f : mesh.faces()) {
        ...
        for(const auto h : CGAL::halfedges_around_face(mesh.halfedge(f), mesh)) {
            ...
            // The face scan reaches every interior edge from both sides, which is where the
            // second, oppositely labelled report of each seam comes from.
            seams.emplace_back(
                LineSegment{xy_of(mesh.point(mesh.source(h))), xy_of(mesh.point(mesh.target(h)))},
                r,
                other);
        }
    }
