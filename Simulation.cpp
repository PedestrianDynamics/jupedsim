...
bool Simulation::InitArgs()
{
    ...
    if(!_config->GetTrajectoriesFile().empty()) {
        ...
        switch(_config->GetFileFormat()) {
            case FORMAT_XML_PLAIN: {
                auto tofile           = std::make_shared<FileHandler>(trajPath.c_str());
                Trajectories * output = new TrajectoriesJPSV05();
    ...
    _gotSources = (bool) distributor->GetAgentsSources()
                      .size(); // did we have any sources? false if no sources
    ...
    if(_building->GetTrainTimeTables().size()) {
        Logging::Info(
            fmt::format("Got {} Train Time Tables", _building->GetTrainTimeTables().size()));
    } else {
        Logging::Warning(fmt::format(
            FMT_STRING("Got {} Train Time Tables"), _building->GetTrainTimeTables().size()));
    }
