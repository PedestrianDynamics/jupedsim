...
void KrauszModel::ComputeNextTimeStep(
    double current,
    double deltaT,
    Building * building,
    int periodic)
...
#pragma omp parallel default(shared) num_threads(nThreads)
    {
        ...
        for(int p = start; p <= end; ++p) {
            ...
            if(normVi > tmp && ped->GetV0Norm() > 0) {
                fprintf(
                    stderr,
                    "KrauszModel::calculateForce() WARNING: actual velocity (%f) of iped %d "
...
inline Point KrauszModel::AccelOscil(Pedestrian * ped) const
{
    ...
    double accel     = omega * omega * amplitude *
                   sin(omega * ped->GetGlobalTime()); //TODO: inclue phase of oscillations
...
Point KrauszModel::ForceInterpolation(
    double v0,
    double K_ij,
    const Point & e,
    double vn,
