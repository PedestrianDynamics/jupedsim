...
#pragma omp parallel for
    for(size_t p = 0; p < allPeds.size(); p++) {
        ...
        if(ped->FindRoute() == -1) {
            Log->Write(
                "ERROR:\tGradientModel::Init() cannot initialise route. ped %d is scheduled for "
                "deletion.\n",
                ped->GetID());
...
void GradientModel::ComputeNextTimeStep(
    double current,
    double deltaT,
    Building * building,
    int periodic)
...
#pragma omp parallel default(shared) num_threads(nThreads)
    {
        ...
        for(int p = start; p <= end; ++p) {
            ...
            if(normVi > HighVel && true) {
                fprintf(
                    stderr,
                    "GradientModel::calculateForce_LC() WARNING: actual velocity (%f) of iped %d "
            ...
            for(int i = 0; i < size; i++) {
                ...
                if((minAddress > neighbours[i]) || (maxAddress < neighbours[i])) {
                    std::cerr << "## Skiped " << i << " of " << size << " #### " << ped1 << " "
                              << minAddress << " " << maxAddress << std::endl;
                    continue;
                }
        ...
        for(int p = start; p <= end; ++p) {
            ...
            Point movDirection = (result_acc[p - start].Norm() > 1) ?
                                     result_acc[p - start].Normalized() :
                                     result_acc[p - start];
            Point toTarget = (_direction->GetTarget(building->GetRoom(ped->GetRoomID()), ped));
            toTarget       = toTarget - ped->GetPos();
            if(toTarget.NormSquare() ==
               0.) { // this if overcomes shortcomming of floorfield (neggrad[targetpoints] == Point(0., 0.))
            ...
            if(desired_speed > 0.) {
                oldMov =
                    ped->GetV() /
                    desired_speed; // (GetV() returns a vector: distance travelled in one time-unit (s)
            ...
            oldMov = (oldMov.Norm() > 1.) ?
                         oldMov.Normalized() :
                         oldMov; //on the safe side ... should not be necessary as it must be [0, 1]
            Point diff =
                (oldMov - movDirection) * (.2); // .2 also 80% alte Richtung, 20% neue Richtung
            ...
            if(auto dirff =
                   dynamic_cast<DirectionFloorfield *>(_direction->GetDirectionStrategy().get())) {
                dir2Wall      = dirff->GetDir2Wall(ped);
                distance2Wall = dirff->GetDistance2Wall(ped);
            } else if(
                auto dirlocff = dynamic_cast<DirectionLocalFloorfield *>(
                    _direction->GetDirectionStrategy().get())) {
                dir2Wall      = dirlocff->GetDir2Wall(ped);
                distance2Wall = dirlocff->GetDistance2Wall(ped);
            } else if(
                auto dirsublocff = dynamic_cast<DirectionSubLocalFloorfield *>(
                    _direction->GetDirectionStrategy().get())) {
                dir2Wall      = dirsublocff->GetDir2Wall(ped);
                distance2Wall = dirsublocff->GetDistance2Wall(ped);
            } else {
                Log->Write("ERROR: \t GradNav Model (4) requires any floor field (exit-strat "
                           "{6,8,9}). None found!");
            }
            double dotProduct         = 0;
            ...
            if(distance2Wall < _slowDownDistance) {
                ...
                if((dotProduct > 0) &&
                   (distance2Wall < .5 * _slowDownDistance)) { //acute angle && really close to wall
                    movDirection = movDirection -
                                   (dir2Wall * dotProduct); //remove walldirection from movDirection
                    ++(*redircnt);
                }
...
#define DEBUG 0
    ...
    //}
    F_driv =
        ((target.Normalized() * ped->GetV0Norm() - ped->GetV()) * ped->GetMass()) / ped->GetTau();
    ...
    printf(
        "mass=%f, v0norm=%f, v=%f, e0Norm=%f, tau=%f\n",
        ped->GetMass(),
        ped->GetV0Norm(),
        v,
        e0norm,
