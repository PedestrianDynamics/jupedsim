...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct Point_V *p)
{
	int i, bucket;
	struct Halfedge *he;
...
struct Site * VoronoiDiagramGenerator::leftreg(struct Halfedge *he)
{
	if(he -> ELedge == (struct Edge *)NULL) 
		return(bottomsite);
	return( he -> ELpm == lee ?
		he -> ELedge -> reg[lee] : he -> ELedge -> reg[ree]);
}
...
struct Site * VoronoiDiagramGenerator::rightreg(struct Halfedge *he)
{
	...
	return( he -> ELpm == lee ? he -> ELedge -> reg[ree] : he -> ELedge -> reg[lee]);
}
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct Point_V *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	double d, xint, yint;
	...
	if ((right_of_site && el -> ELpm == lee) || (!right_of_site && el -> ELpm == ree))
		return ((struct Site *) NULL);
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct Point_V *p)
{
	struct Edge *e;
	struct Site *topsite;
	...
	if(right_of_site && el -> ELpm == lee) return(1);
	if(!right_of_site && el -> ELpm == ree) return (0);
	...
	return (el->ELpm==lee ? above : !above);
}
...
void VoronoiDiagramGenerator::endpoint(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
	if(e -> ep[ree-lr]== (struct Site *) NULL)
		return;
...
struct Point_V VoronoiDiagramGenerator::PQ_min()
{
	struct Point_V answer;
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
	...
	while(1)
	{
		...
		{/* new site is smallest - this is a site event*/
			...
			bisector = HEcreate(e, lee);					//create a new HalfEdge, setting its ELpm field to 0
			...
			lbnd = bisector;						
			bisector = HEcreate(e, ree);					//create a new HalfEdge, setting its ELpm field to 1
		...
		{	
			...
			pm = lee;								//set the pm variable to zero
			...
			endpoint(e, ree-pm, v);					//set one endpoint to the new edge to be the vector point 'v'.
...
{
	struct Point_V *s1 = (Point_V*)p1, *s2=(Point_V*)p2;
