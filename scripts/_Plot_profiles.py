...
if __name__ == '__main__':
    ...
    print("geometry: X [%.1f, %.1f], Y=[%.1f, %.1f]" %(geominX, geomaxX, geominY, geomaxY))

    density_files = os.path.join(pathfile, "density", "*%s*%d.dat" % (nametraj,beginsteady))
    ...
    im = plt.imshow(density, cmap=cm.jet, interpolation='nearest',origin='lower',vmin=0,vmax=np.amax(density), extent=[geominX,geomaxX,geominY,geomaxY])
    ax1.set_xlabel("x [m]")
    ax1.set_ylabel("y [m]")
    ...
    plt.rc("font", size=40)
    for j in range(beginsteady,endsteady):
        velocity_file = os.path.join(pathfile, "velocity", "Prf_v_%s_id_1_%.5d.dat" % (nametraj, j) )
        print("loading: %s" % velocity_file)
        velocity += np.loadtxt(velocity_file)

    velocity=velocity/(endsteady-beginsteady)
    im = plt.imshow(velocity, cmap=cm.jet, interpolation='nearest',origin='lower',vmin=0,vmax=np.amax(velocity), extent=[geominX,geomaxX,geominY,geomaxY])
    ax1.set_xlabel("x [m]")
    ax1.set_ylabel("y [m]")
    divider = make_axes_locatable(ax1)
    cax = divider.append_axes("right", size="2.5%", pad=0.3)
...
#---------------------------------flow profile-------------------------------------------------------------
    ...
    im = plt.imshow(flow, cmap=cm.jet, interpolation='nearest',origin='lower',vmin=0,vmax=np.amax(flow), extent=[geominX,geomaxX,geominY,geomaxY])
    ax1.set_xlabel("x [m]")
    ax1.set_ylabel("y [m]")
    divider = make_axes_locatable(ax1)
    cax = divider.append_axes("right", size="2.5%", pad=0.3)
