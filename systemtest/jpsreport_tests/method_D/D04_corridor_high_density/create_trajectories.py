...
startPosX = 0.0  # start position, upper left corner of the grid
startPosY = 3.0
numPedsX = 6  # number of columns of pedestrians in the grid
numPedsY = 6  # number of rows of pedestrians in the grid
dist = 0.5  # distance between the pedestrians (vertically and horizontally)
...
for frame in range(sim_frames):
    ...
    for i in range(numPedsX):
        for j in range(numPedsY):
            ...
            xPos = startPosX + i * dist
            yPos = startPosY - j * dist

            f.write(
                "{:6d}\t{:6d}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:d}\n".format(id, frame + 1, xPos, yPos,
                                                                                              0.0, a, b,
                                                                                              angle, color))
