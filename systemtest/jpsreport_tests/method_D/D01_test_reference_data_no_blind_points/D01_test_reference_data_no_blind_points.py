...
def runtest(inifile, trajfile):
    ...
    data_2_filename = os.path.join('./Output',
                                   'Fundamental_Diagram',
                                   'IndividualFD',
                                   'IFD_rho_v_Voronoi_Voronoi_%s_id_5.dat'%trajfile
                                   )
    ...
    if x_p_value > alpha:
        ...
        logging.info('--> x-coordinate: different dist -- p-value: {0:.4f}'.
                     format(x_p_value))
        x_exit = False
    ...
    if y_p_value > alpha:
        ...
        logging.info('--> y-coordinate: different dist -- p-value: {0:.4f}'.
                     format(y_p_value))
        y_exit = False
    ...
    if z_p_value > alpha:
        ...
        logging.info('--> z-coordinate: different dist -- p-value: {0:.4f}'.
                     format(z_p_value))
        z_exit = False
