...
int QuickestPathRouter::FindNextExit(Pedestrian* ped){
	...
	for(unsigned int i=0;i<accessPointsInSubRoom.size();i++){
		...
		nextDestination = GetQuickestRoute(ped,pAccessPoints[apID]);
		//printf("[%d === %d]\n",dum,nextDestination);
	...
	ped->SetSmoothTurning(true);
	ped->SetExitLine(GetAllGoals()[bestAPsID]);
...
double QuickestPathRouter::GetJamSizeAtExit(int exitID){
	...
	SubRoom* sbr1 = GetAllGoals()[exitID]->GetSubRoom1();
	SubRoom* sbr2 = GetAllGoals()[exitID]->GetSubRoom2();
...
void QuickestPathRouter::ExpandGraph(){
	...
	for(unsigned int i=0;i<pAccessPoints.size();i++){
		vector<AccessPoint*>toBeDeleted;
		AccessPoint* tmp=pAccessPoints[i];
		//int tmpID=tmp->GetID();
		...
		for(unsigned int j=0;j<aps.size();j++)
		{
			...
			double dist1=pAccessPoints[bestID]->GetDistanceTo(tmp)+tmp->GetDistanceTo(tmp1);
...
void QuickestPathRouter::SelectReferencePedestrian(Pedestrian* me, Pedestrian** myref, int exitID, int* flag){
	...
	*flag=FREE_EXIT; // assume free exit
	...
	Crossing* crossing=GetAllGoals()[exitID];
...
int QuickestPathRouter::GetCommonDestinationCount(AccessPoint* ap1, AccessPoint* ap2){
	...
	//	if((ap1->GetID()==863)&&(ap2->GetID()==911)) {
	//		for(unsigned int i=0;i<common.size();i++){
	//			cout<<" common: "<<common[i]->GetID()<<endl;
	//		}
	//	}

	//special case (again) in the promenade
	Room* room = GetAllGoals()[ap1->GetID()]->GetRoom1();
	if(room->GetCaption()=="010") {
		//identify the subroom
		Crossing* c1 =GetAllGoals()[ap1->GetID()];
		Crossing* c2=GetAllGoals()[ap2->GetID()];
		SubRoom* sub = GetCommonSubRoom(c1,c2);
		if(sub && (sub->GetSubRoomID()==0)){
			// connect based on the size of the crossings
			if  (c1->Length()<2)  return 2; // any number >0
		}
	}

	if(room->GetCaption()=="030") {
		//identify the subroom
		Crossing* c1 =GetAllGoals()[ap1->GetID()];
		Crossing* c2=GetAllGoals()[ap2->GetID()];
		SubRoom* sub = GetCommonSubRoom(c1,c2);
		if(sub && (sub->GetSubRoomID()==1)){
			// connect based on the size of the crossings
			if  (c1->Length()<2)  return 2; // any number >0
		}
	}
...
bool QuickestPathRouter::IsDirectVisibilityBetween(Pedestrian* ped, Pedestrian* ref){
	...
	Crossing* ignore_crossing=GetAllGoals()[ref->GetExitIndex()];
...
int QuickestPathRouter::isCongested(Pedestrian* ped){
	...
	pBuilding->GetGrid()->GetNeighbourhood(ped,neighbourhood);
...
double QuickestPathRouter::GetEstimatedTravelTimeVia(Pedestrian* ped, int exitid){
	...
	AccessPoint* ap=pAccessPoints[exitid];
...
void QuickestPathRouter::Redirect(Pedestrian* ped){
	...
	Room* room=pBuilding->GetRoom(ped->GetRoomID());
	...
	for(unsigned int g=0;g<goals.size();g++){
		...
		// segment connecting the two APs/goals
		const Point& p1 = GetAllGoals()[exitid]->GetCentre();
		const Point& p2 = ped->GetPos();
		Line segment = Line(p1,p2);
	...
	if(quickest!=preferredExit){
		...
		if (cba>0.010){
			...
			ped->SetExitLine(GetAllGoals()[quickest]);
		}
