...
void FloorfieldViaFMTrips::getDirectionToUID(
    int destID,
    const long int key,
    Point & direction,
    int mode)
...
#pragma omp critical(floorfieldsBeingCalculated)
            {
                if(_floorfieldsBeingCalculated.count(destID) != 1) {
                    Log->Write(
                        "ERROR: FloorfieldViaFMTrips::getDirectionToUID: key %d was calculating FF "
                        "for destID %d, but it was removed from floorfieldsBeingCalculated "
                        "meanwhile",
                        key,
                        destID);
                }
...
void FloorfieldViaFMTrips::createMapEntryInLineToGoalID(const int goalID, bool isInside)
{
    ...
    double * localcostptr;
    if(goalID < 0) {
        Log->Write("WARNING: \t goalID was negative in "
                   "FloorfieldViaFMTrips::createMapEntryInLineToGoalID");
        return;
    }
    if(!_building->GetFinalGoal(goalID)) {
        Log->Write(
            "WARNING: \t goalID was unknown in FloorfieldViaFMTrips::createMapEntryInLineToGoalID");
        return;
    }
...
double
FloorfieldViaFMTrips::getCostToDestination(const int destID, const Point & position, int mode)
{
 ...
 * \warning
 */
void FloorfieldViaFMTrips::parseBuilding(
    const Building * const buildingArg,
 ...
 * \warning
 */
void FloorfieldViaFMTrips::parseBuildingForExits(
    const Building * const buildingArg,
    const double stepSizeX,
    const double stepSizeY)
{
...
void FloorfieldViaFMTrips::prepareForDistanceFieldCalculation(const bool onlyRoomsWithExits)
...
template <typename T>
void FloorfieldViaFMTrips::drawLinesOnGrid(
...
void FloorfieldViaFMTrips::setSpeedThruPeds(
...
void FloorfieldViaFMTrips::calculateFloorfield(
    ...
    double * speedarray)
{
    ...
    for(long int i = 0; i < _grid->GetnPoints(); ++i) {
        if(flag[i] == FM_FINAL) {
            ...
            checkNeighborsAndAddToNarrowband(
                trialqueue, newTrialPTrips, [&](TrialPTrips TrialPTripsArg) {
                    this->calcFloorfield(TrialPTripsArg);
                });
        }
    }
    ...
    while(!trialqueue.empty()) {
        ...
        trialqueue.pop();
        TrialPTrips newTrialPTrips =
            TrialPTrips(keyOfSmallest, costarray, speedarray, flag, neggradarray);
        checkNeighborsAndAddToNarrowband(
            trialqueue, newTrialPTrips, [&](TrialPTrips TrialPTripsArg) {
                this->calcFloorfield(TrialPTripsArg);
            });
...
void FloorfieldViaFMTrips::calculateDistanceField(const double thresholdArg)
    ...
    for(long int i = 0; i < _grid->GetnPoints(); ++i) {
        ...
        if((_gcode[i] == WALL) || (_gcode[i] == CLOSED_TRANSITION) ||
           (_gcode[i] == CLOSED_CROSSING)) {
            ...
            checkNeighborsAndAddToNarrowband(trialqueue, newP, [&](TrialPTrips TrialPTripsArg) {
                this->calcDist2Wall(TrialPTripsArg);
            });
        }
    }
    ...
    while(!trialqueue.empty()) {
        ...
        if((thresholdArg > 0) &&
           (_dist2Wall[keyOfSmallest] >
            ...
            TrialPTrips smallestP =
                TrialPTrips(keyOfSmallest, _dist2Wall, _speedInitial, flag, _dirToWall);
            checkNeighborsAndAddToNarrowband(
                trialqueue, smallestP, [&](TrialPTrips TrialPTripsArg) {
                    this->calcDist2Wall(TrialPTripsArg);
                });
        }
    }
...
void FloorfieldViaFMTrips::checkNeighborsAndAddToNarrowband(
    std::priority_queue<TrialPTrips, std::vector<TrialPTrips>, std::greater<TrialPTrips>> &
        trialqueue,
    TrialPTrips keyP,
...
void FloorfieldViaFMTrips::calcFloorfield(TrialPTrips newPoint)
{
    ...
    if((col == DBL_MAX) && (row == DBL_MAX)) {
        std::cerr
            << "Issue 175 in FloorfieldViaFMTrips: invalid combination of row,col (both on max)"
            << std::endl;
        return;
    }
...
inline double FloorfieldViaFMTrips::twosidedCalc(double x, double y, double hDivF)
{ //on error return -2
...
void FloorfieldViaFMTrips::testoutput(
...
void FloorfieldViaFMTrips::writeFF(const std::string & filename, std::vector<int> targetID)
{
    ...
    Log->Write(
        "FloorfieldViaFMTrips::writeFF(): writing to file %s: There are %d targets.",
        filename.c_str(),
        targetID.size());
...
void CentrePointFFViaFMTrips::getDirectionToUID(
    int destID,
    const long int key,
    Point & direction,
    int mode)
