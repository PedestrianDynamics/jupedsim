    ...
    for(TiXmlElement * xRegion = xRegionsNode->FirstChildElement("region"); xRegion;
        ...
        if(!xConnectionsNode) {
            ...
            for(TiXmlElement * xConnection = xConnectionsNode->FirstChildElement("connection");
                ...
                AIConnection connection = AIConnection(
                    std::stoi(idC), captionC, typeC, std::stoi(landmark1), std::stoi(landmark2));
...
void AIBrainStorage::CreateCortex(BStorageKeyType ped)
{
    ...
    _corteces.insert(std::make_pair(
        ped,
        std::unique_ptr<Cortex>(new Cortex(
            _building,
            ped,
            &_visibleEnv,
...
void AIBrainStorage::ParseSigns()
{
    ...
    if(xRootNode->Attribute("unit"))
        if(std::string(xRootNode->Attribute("unit")) != "m") {
            Log->Write(
                "WARNING:\tOnly the unit m (meters) is supported. \n\tYou supplied [%s]",
                xRootNode->Attribute("unit"));
            Log->Write("No signs specified");
            return;
        }
    ...
    for(TiXmlElement * xSign = xSignsNode->FirstChildElement("sign"); xSign;
        ...
        _visibleEnv.AddSign(
            _building->GetAllRooms().at(std::stod(room_id)).get(),
            Sign(
                std::stod(id),
                std::stod(room_id),
                Point(std::stod(px), std::stod(py)),
                std::stod(alpha),
                std::stod(alphaPointing)));
    }
}
