...
const std::unordered_map<ptrFloor, BoostPolygon> & VisualSystem::UpdateSeenEnv()
{
    ...
    try {
        ...
        boost::geometry::intersection(
            _seenEnvironment[_building->GetRoom(_ped->GetRoomID())], rectangle, output2);
        if(!output2.empty())
        ...
        boost::geometry::union_(
            _seenEnvironment[_building->GetRoom(_ped->GetRoomID())], _currentEnv, output);
        if(!output.empty())
    ...
    std::string str2 = "./isovists/seen_env_" + std::to_string(_ped->GetID()) + "_" +
                       std::to_string((int) (std::round(_ped->GetGlobalTime() * 100))) + ".txt";
    myfile2.open(str2);
    for(auto it1 = boost::begin(boost::geometry::exterior_ring(
            _seenEnvironment.at(_building->GetRoom(_ped->GetRoomID()))));
        it1 != boost::end(boost::geometry::exterior_ring(
                   _seenEnvironment.at(_building->GetRoom(_ped->GetRoomID()))));
        ++it1) {
    ...
    std::string str3 = "./isovists/real_pos" + std::to_string(_ped->GetID()) + "_" +
                       std::to_string((int) (std::round(_ped->GetGlobalTime() * 100))) + ".txt";
    myfile3.open(str3);
    myfile3 << std::to_string(_ped->GetPos()._x) << " " << std::to_string(_ped->GetPos()._y)
            << std::endl;
    myfile3.close();
...
const BoostPolygon & VisualSystem::UpdateCurrentEnvironment()
{
    ...
    const Arrangement_2 & arr =
        _env->GetCGALGeometry().GetVisEnvCGAL(_building->GetRoom(_ped->GetRoomID()));
    ...
    Arrangement_2::Face_handle fh =
        _env->GetCGALGeometry()
            .GetTEV(_building->GetRoom(_ped->GetRoomID()))
            ->compute_visibility(q, *face, output_arr); //query_point, he, output_arr);
    ...
    std::ofstream myfile;
    std::string str = "./isovists/current_env_" + std::to_string(_ped->GetID()) + "_" +
                      std::to_string((int) (std::round(_ped->GetGlobalTime() * 100))) + ".txt";
    myfile.open(str);
...
std::vector<NavLine> VisualSystem::GetPossibleNavLines(const std::vector<Point> & envPolygon) const
{
    ...
    for(size_t i = 1; i < visPolygon.size(); ++i) {
        ...
        for(Linestring lineS : _env->GetAllWallsOfEnv(_building->GetRoom(_ped->GetRoomID()))) {
            ...
            if(boost::geometry::distance(center, lineS) <=
               0.15) // && boost::geometry::distance(visPolygon[i],lineS)<=0.15)
            {
        ...
        if(intersect ==
           false) //&& (IsInSubRoom(visPolygon[i-1])!=nullptr || IsInSubRoom(visPolygon[i])!=nullptr))
        {
...
std::vector<NavLine> VisualSystem::FindTransitions() const
{
    ...
    for(int transID : _building->GetRoom(_ped->GetRoomID())->GetAllTransitionsIDs()) {
        Point normal = Point(
            -(_building->GetTransitionByUID(transID)->GetPoint2() -
              _building->GetTransitionByUID(transID)->GetPoint1())
                 ._y,
            (_building->GetTransitionByUID(transID)->GetPoint2() -
             _building->GetTransitionByUID(transID)->GetPoint1())
                ._x);
        ...
        if(boost::geometry::within(
               _building->GetTransitionByUID(transID)->GetCentre() + normal * 0.5, _currentEnv) ||
...
{
    for(auto it2 = _building->GetRoom(_ped->GetRoomID())->GetAllSubRooms().begin();
        it2 != _building->GetRoom(_ped->GetRoomID())->GetAllSubRooms().end();
        ++it2) {
        BoostPolygon boostP;
    ...
    for(int transID : transIDs) {
        if(_building->GetTransitionByUID(transID)->GetOtherRoom(_ped->GetRoomID()))
            adjacentRooms.push_back(
                _building->GetTransitionByUID(transID)->GetOtherRoom(_ped->GetRoomID()));
    }
...
void VisualSystem::GetVisibleSigns()
{
    ...
    const std::vector<Sign> * signsInCurrentFloor =
        _env->GetSignsOfFloor(_building->GetAllRooms().at(_ped->GetRoomID()).get());
    ...
    for(const Sign & sign : *signsInCurrentFloor) {
        ...
        if(SignLegible(sign)) {
            ...
            if(std::find(_visibleSigns.begin(), _visibleSigns.end(), &sign) ==
               _visibleSigns.end()) {
        ...
        // if sign not anymore in field of view
            ...
            std::find(_visibleSigns.begin(), _visibleSigns.end(), &sign) != _visibleSigns.end()) {
            _visibleSigns.erase(std::find(_visibleSigns.begin(), _visibleSigns.end(), &sign));

            if(std::find(_detectedSigns.begin(), _detectedSigns.end(), &sign) !=
               _detectedSigns.end())
                _detectedSigns.erase(
                    std::find(_detectedSigns.begin(), _detectedSigns.end(), &sign));
        }
    }
}
...
bool VisualSystem::TryToDetectSign(const Sign & sign, double headDir, bool hardlyVisSign)
{
    ...
    int number =
        distribution(_building->GetConfig()->GetRandomNumberGenerator()->GetRandomEngine());
