...
CGALGeometry::CGALGeometry(const std::unordered_map<ptrFloor, std::vector<const Line *>> & allWalls)
{
    ...
    for(auto it = allWalls.begin(); it != allWalls.end(); ++it) {
        ...
        while(!separatedPolygons.empty()) {
            ...
            Log->Write(
                std::to_string(separatedPolygons.size()) + " of " + std::to_string(walls.size()));
        }
        ...
        Point_2 pb1 = Point_2(
            it->first->GetBoundaryVertices()[0]._x - 1, it->first->GetBoundaryVertices()[0]._y - 1);
        Point_2 pb2 = Point_2(
            it->first->GetBoundaryVertices()[3]._x + 1, it->first->GetBoundaryVertices()[3]._y - 1);
        Point_2 pb3 = Point_2(
            it->first->GetBoundaryVertices()[2]._x + 1, it->first->GetBoundaryVertices()[2]._y + 1);
        Point_2 pb4 = Point_2(
            it->first->GetBoundaryVertices()[1]._x - 1, it->first->GetBoundaryVertices()[1]._y + 1);
