    ...
    for(auto it = _b->GetAllRooms().begin(); it != _b->GetAllRooms().end(); ++it) {
        ...
        if(it->second->GetID() == 6 || it->second->GetID() == 0 || it->second->GetID() == 11) {
            for(int transID : it->second->GetAllTransitionsIDs()) {
                ...
                bool within  = false;
                Point normal = Point(
                    -(_b->GetTransitionByUID(transID)->GetPoint2() -
                      _b->GetTransitionByUID(transID)->GetPoint1())
                         ._y,
                    (_b->GetTransitionByUID(transID)->GetPoint2() -
                     _b->GetTransitionByUID(transID)->GetPoint1())
                        ._x);
                ...
                if(within == true) {
                    ...
                    boost::geometry::append(
                        lineS, _b->GetTransitionByUID(transID)->GetPoint1() - normal * 0.5);
                    boost::geometry::append(
                        lineS, _b->GetTransitionByUID(transID)->GetPoint2() - normal * 0.5);
                    lines.push_back(lineS);
                    Line * newLine = new Line(
                        _b->GetTransitionByUID(transID)->GetPoint1() - normal * 0.5,
                        _b->GetTransitionByUID(transID)->GetPoint2() - normal * 0.5);
                    wallsOfOneRoom.push_back(newLine);

                    Linestring lineS1;
                    boost::geometry::append(
                        lineS1, _b->GetTransitionByUID(transID)->GetPoint1() - normal * 0.5);
                    ...
                    Linestring lineS2;
                    boost::geometry::append(
                        lineS2, _b->GetTransitionByUID(transID)->GetPoint2() - normal * 0.5);
