...
std::vector<const NavLine *>
Cortex::SortConGeneralKnow(const std::vector<const NavLine *> & navLines)
{
        ...
        if(!transition) {
            if(_ped->GetRoomID() == 11) {
                if(IsInsideRectangle(
                       navLine->GetCentre(), Point(7.52, 52.19), Point(11.31, 46.0)) ||
                   IsInsideRectangle(
                       navLine->GetCentre(), Point(20.25, 68.38), Point(27.2, 64.91)) ||
                   IsInsideRectangle(
                       navLine->GetCentre(), Point(4.48, 79.5), Point(13.78, 73.14)) ||
                   IsInsideRectangle(
                       navLine->GetCentre(), Point(-7.06, 54.69), Point(-1.48, 50.48))) {
            ...
            if(transition->GetOtherRoom(_ped->GetRoomID())->GetZPos() >
               _b->GetRoom(_ped->GetRoomID())->GetZPos()) {
    ...
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesOutside),
        std::end(sortedNavLinesOutside));
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesLobbies),
        std::end(sortedNavLinesLobbies));
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesStairsUp),
        std::end(sortedNavLinesStairsUp));
    sortedNavLines.insert(
        std::end(sortedNavLines), std::begin(sortedNavLinesStairs), std::end(sortedNavLinesStairs));
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesCirculs),
        std::end(sortedNavLinesCirculs));
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesCommons),
        std::end(sortedNavLinesCommons));
    sortedNavLines.insert(
        std::end(sortedNavLines),
        std::begin(sortedNavLinesStairsDown),
        std::end(sortedNavLinesStairsDown));
    sortedNavLines.insert(
        std::end(sortedNavLines), std::begin(sortedNavLinesTrains), std::end(sortedNavLinesTrains));
...
const NavLine Cortex::FindApprCrossing(const std::vector<NavLine> & navLines)
{
    ...
    std::shuffle(
        shuffledNavLines.begin(),
        shuffledNavLines.end(),
        _b->GetConfig()->GetRandomNumberGenerator()->GetRandomEngine());

    // knowledge generalized <- signage  <- Cognitive Map <- Random shuffle
    const NavLine * nextNvLine =
        SortConGeneralKnow(SortConSignage(_cMap.SortConLeastAngle(
                               _cMap.GetNextTarget(), shuffledNavLines, _lastBestVisNavLine)))
            .front();
...
const NavLine
Cortex::FindApprVisibleCrossing(const NavLine & navLine, const std::vector<NavLine> & navLines)
{
    // if transition or outside is in sight
    Point normal = Point(
        -(navLine.GetPoint2() - navLine.GetPoint1())._y,
        (navLine.GetPoint2() - navLine.GetPoint1())._x);
    ...
    for(int transID : transIDs) {
        if(*(_b->GetTransitionByUID(transID)) == navLine) {
            ...
            if(dirVec1.Norm() > dirVec2.Norm())
                targetToHead =
                    Line(navLine.GetPoint1() + normal * 0.5, navLine.GetPoint2() + normal * 0.5);
            else
                targetToHead =
                    Line(navLine.GetPoint1() - normal * 0.5, navLine.GetPoint2() - normal * 0.5);
    ...
    // not a transition
    if(_lastBestVisNavLine.GetPoint1() == Point(0, 0) &&
       _lastBestVisNavLine.GetPoint2() == Point(0, 0)) {
    ...
    if(statFoundBestChoice) {
        ...
        for(const NavLine & candidate : navLines) {
            ...
            if(diffAlpha < M_PI / 3.0 &&
               candidate.DistTo(navLine.GetCentre()) <= minDistance * 0.8) {
                minDistance         = candidate.DistTo(navLine.GetCentre());
                _lastBestVisNavLine = candidate;
                statFound           = true;
            }
...
std::vector<const NavLine *> Cortex::SortConSignage(const std::vector<const NavLine *> & navLines)
{
    ...
    if(sortedNavLines.size() > 1) {
        ...
        size_t index = std::min_element(orthogonalDistances.begin(), orthogonalDistances.end()) -
                       orthogonalDistances.begin();
...
const Sign * Cortex::DetermineDecisiveSign(const std::vector<const Sign *> & signs)
{
    ...
    for(size_t i = 0; i < cSigns.size(); ++i) {
        ...
        for(size_t j = 0; j < cSigns.size(); ++j) {
            if(i != j) {
                ...
                cAngles.push_back(std::fabs(
                    cSigns[i]->GetAlphaPointing() -
                    std::atan2(vec_ij._y, vec_ij._x) * 180.0 / M_PI));
            }
        }
    ...
    if(signsNotPointing.size() > 1) {
        std::shuffle(
            signsNotPointing.begin(),
            signsNotPointing.end(),
            _b->GetConfig()->GetRandomNumberGenerator()->GetRandomEngine());
...
double Cortex::OrthogonalDistanceFromTo(
...
{
    if(point._x > leftUp._x && point._x < rightDown._x && point._y > rightDown._y &&
       point._y < leftUp._y)
        return true;
    else
        return false;
