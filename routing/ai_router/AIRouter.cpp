...
int AIRouter::FindExit(Pedestrian * p)
{
    ...
    if(std::fmod(std::roundf(p->GetGlobalTime() * 100), std::roundf(1.0 / AI_UPDATE_RATE * 100)) ==
       0.0) // && p->GetGlobalTime()>0)
    {
...
int AIRouter::FindDestination(Pedestrian * p)
{
    ...
    std::vector<NavLine> navLines =
        (*brain_storage)[p]->GetPerceptionAbilities().GetPossibleNavLines(
            (*brain_storage)[p]->GetPerceptionAbilities().GetSeenEnvironmentAsPointVec(
                building->GetRoom(p->GetRoomID())));

    NavLine nextNvLine = (*brain_storage)[p]->FindApprCrossing(navLines);

    std::ofstream myfile2;
    std::string str2 = "./isovists/navLinesBefore_pos" + std::to_string(p->GetID()) + "_" +
                       std::to_string((int) (std::round(p->GetGlobalTime() * 100))) + ".txt";
    myfile2.open(str2);
    myfile2 << std::to_string(nextNvLine.GetCentre()._x) << " "
            << std::to_string(nextNvLine.GetCentre()._y) << std::endl;
    myfile2.close();
    ...
    std::ofstream myfile;
    std::string str = "./isovists/navLines_pos" + std::to_string(p->GetID()) + "_" +
                      std::to_string((int) (std::round(p->GetGlobalTime() * 100))) + ".txt";
    myfile.open(str);
    myfile << std::to_string(nextNvLine.GetCentre()._x) << " "
           << std::to_string(nextNvLine.GetCentre()._y) << std::endl;
    myfile.close();
...
bool AIRouter::Init(Building * b)
{
    ...
    if(getOptions().find("CognitiveMapFiles") == getOptions().end() &&
       getOptions().find("SignFiles") == getOptions().end())
        ...
        brain_storage = std::unique_ptr<AIBrainStorage>(
            new AIBrainStorage(building, "", getOptions().at("SignFiles")[0]));
    else if(getOptions().find("SignFiles") == getOptions().end())
        brain_storage = std::unique_ptr<AIBrainStorage>(
            new AIBrainStorage(building, getOptions().at("CognitiveMapFiles")[0]));
    else
        brain_storage = std::unique_ptr<AIBrainStorage>(new AIBrainStorage(
            building, getOptions().at("CognitiveMapFiles")[0], getOptions().at("SignFiles")[0]));
