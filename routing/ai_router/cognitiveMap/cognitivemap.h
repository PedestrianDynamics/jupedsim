...
public:
    ...
    TriggerAssociations(const std::vector<const AILandmark *> & landmarks);

    std::vector<const NavLine *> SortConLeastAngle(
    ...
    //Find targets
    const AILandmark *
    FindConnectionPoint(const AIRegion * currentRegion, const AIRegion * targetRegion) const;
    ...
    const AILandmark *
    FindBestRouteFromOneOf(const std::vector<const AILandmark *> & nearLandmarks);
