...
std::vector<const AILandmark *>
AICognitiveMap::TriggerAssociations(const std::vector<const AILandmark *> & landmarks)
{
...
std::vector<const NavLine *> AICognitiveMap::SortConLeastAngle(
    const AILandmark * landmark,
    const std::vector<NavLine> & navLines,
    const NavLine & focus)
{
    ...
    if(_lastBestChoice.GetPoint1() == Point(0, 0) && _lastBestChoice.GetPoint2() == Point(0, 0)) {
        ...
        for(const NavLine * navline : cNavLines) {
            Point pCloser =
                GetCloserPoint(_ped->GetPos(), navline->GetPoint1(), navline->GetPoint2());
            Point pCloserBestChoice = GetCloserPoint(
                _ped->GetPos(), _lastBestChoice.GetPoint1(), _lastBestChoice.GetPoint2());
    ...
    if(landmark) {
        ...
        Point onEllipse = Point(
            landmark->GetPosInMap()._x + landmark->GetA() * std::cos(angleVecPedLCenter),
            landmark->GetPosInMap()._y + landmark->GetB() * std::sin(angleVecPedLCenter));
    ...
    while(cNavLines.size() > 0) {
        ...
        for(auto it = cNavLines.begin(); it != cNavLines.end(); ++it) {
            double alpha2 = std::atan2(
                ((*it)->GetCentre() - _ped->GetPos())._y, ((*it)->GetCentre() - _ped->GetPos())._x);
...
std::vector<const AILandmark *>
AICognitiveMap::GetLandmarksConnectedWith(const AILandmark * landmark) const
{
    ...
    if(_currentRegion != nullptr) {
        for(const AILandmark & landmark : _currentRegion->GetLandmarks()) {
            if(boost::geometry::intersects(landmark.GetRealPos(), boostPolygon)) {
                if(std::find(
                       _landmarksRecentlyVisited.begin(),
                       _landmarksRecentlyVisited.end(),
                       &landmark) == _landmarksRecentlyVisited.end()) {
...
const AILandmark * AICognitiveMap::FindConnectionPoint(
    const AIRegion * currentRegion,
    const AIRegion * targetRegion) const
{
    ...
    for(const AIRegion & region : _regions) {
        for(const AILandmark & landmark : region.GetLandmarks()) {
            if(landmark.GetType() == "main" &&
               std::find(
                   _landmarksRecentlyVisited.begin(), _landmarksRecentlyVisited.end(), &landmark) ==
                   _landmarksRecentlyVisited.end()) {
...
{
    std::vector<const AILandmark *> landmarksConnectedToTarget =
        FindLandmarksConnectedToTarget(target);
...
std::vector<const AILandmark *>
AICognitiveMap::FindLandmarksConnectedToTarget(const AILandmark * target)
{
...
const AILandmark *
AICognitiveMap::FindBestRouteFromOneOf(const std::vector<const AILandmark *> & nearLandmarks)
{
    const AILandmark * bestChoice = nullptr;
    double minDistance            = FLT_MAX;
    double cDistance;
    for(const AILandmark * landmark : nearLandmarks) {
        cDistance = (_ped->GetPos() - landmark->GetRandomPoint()).Norm() +
                    _currentRegion->PathLengthFromLandmarkToTarget(landmark, _nextTarget).second;
    ...
    std::vector<const AILandmark *> landmarksOnShortestPath =
        _currentRegion->PathLengthFromLandmarkToTarget(bestChoice, _nextTarget).first;

    for(int i = landmarksOnShortestPath.size() - 1; i >= 0; --i) {
        if(std::find(
               _landmarksRecentlyVisited.begin(),
               _landmarksRecentlyVisited.end(),
               landmarksOnShortestPath[i]) == _landmarksRecentlyVisited.end()) {
...
const AILandmark *
AICognitiveMap::GetNearestMainTarget(const std::vector<const AILandmark *> & mainTargets)
{
    ...
    for(const AILandmark * mainDest : mainTargets) {
        ...
        double cDistance =
            (_ped->GetPos() - cLandmark->GetRandomPoint()).Norm() +
            _currentRegion->PathLengthFromLandmarkToTarget(cLandmark, mainDest).second;
