...
AILandmarkNetwork::AILandmarkNetwork(
    const AIRegion * region,
    const AILandmarks & landmarks,
    const std::vector<AIConnection> & connections)
{
    _region = region;
    _graph  = Graph();
...
void AILandmarkNetwork::AddConnection(const AIConnection * connection)
{
    ...
    Point vector = landmarkA->GetRandomPoint() -
                   landmarkB->GetRandomPoint(); //->GetRandomPoint()-landmarkB->GetRandomPoint();
...
std::pair<std::vector<const AILandmark *>, double> AILandmarkNetwork::LengthofShortestPathToTarget(
    const AILandmark * landmark,
    const AILandmark * target) const
{
    if(landmark == target)
        return std::make_pair<std::vector<const AILandmark *>, double>(
            std::vector<const AILandmark *>{landmark}, 0.0);
    ...
    boost::dijkstra_shortest_paths(
        ...
        boost::predecessor_map(
            boost::make_iterator_property_map(p.begin(), get(boost::vertex_index, _graph)))
            .distance_map(
                boost::make_iterator_property_map(d.begin(), get(boost::vertex_index, _graph))));
