...
std::pair<std::vector<const AILandmark *>, double> AIRegion::PathLengthFromLandmarkToTarget(
...
void AIRegion::RemoveConnections(const AILandmark * landmark)
{
    for(auto it = _connections.begin(); it != _connections.end(); ++it) {
        if(it->GetLandmarkIds().first == landmark->GetId() ||
           it->GetLandmarkIds().second == landmark->GetId()) {
