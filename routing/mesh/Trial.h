...
class
    Trial // father := smaller; child := bigger (in terms of cost); cost/speed := ptr to its cost/speed
{
public:
    long int key;
    int * flag;
    ...
    void insert(Trial *& smallest, Trial *& biggest, Trial * add)
    {
        if(smallest != nullptr) {
            ...
            smallest =
                add; //wird anstelle des nullptr eingefuegt (smallest ist nicht immer der global smallest sondern auch das rekursionsargument)
            if(biggest ==
               nullptr) { //indicator, dass keine liste existierte, also setze smallest und biggest auf add
                add->father = nullptr;
    ...
    void lift(Trial *& smallest, Trial *& biggest)
    { //only asserts that smallest is at start
        if(biggest != nullptr) {
            Trial * aux;
            aux = biggest->father;
            if(aux != nullptr) {
