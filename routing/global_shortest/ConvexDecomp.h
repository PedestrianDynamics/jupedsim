...
inline void mark_domains(CDT & ct, CDT::Face_handle start, int index, std::list<CDT::Edge> & border)
{
...
inline int test_alpha_shape()
{
    ...
    for(std::list<Point_2>::const_iterator iterator = env.begin(), end = env.end(); iterator != end;
        ++iterator) {
        std::cout << *iterator;
    }
