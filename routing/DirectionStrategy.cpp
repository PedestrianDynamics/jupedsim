    ...
    bool useDistancMap)
{
    ...
    _ffviafm = new FloorfieldViaFM(building, stepsize, stepsize, threshold, useDistancMap, false);
...
Point DirectionLocalFloorfield::GetTarget(Room * room, Pedestrian * ped) const
{
    ...
    if(1) {
        ...
        if(!floorfield->getGrid()->includesPoint(ped->GetPos())) {
            Log->Write("ERROR: \tDirectionLocalFloorfield::GetTarget is accessing wrong "
                       "floorfield. Pedestrian is not inside!");
            p = Point(0.0, 0.0);
            return p;
        }
#endif
...
void DirectionLocalFloorfield::Init(
    ...
    bool useDistanceMap)
{
    ...
    std::chrono::time_point<std::chrono::system_clock> start, end;
    start = std::chrono::system_clock::now();
    Log->Write("INFO: \tCalling Constructor of UnivFFviaFM(Room-scale) in "
               "DirectionLocalFloorfield::Init(...)");

    for(auto & roomPair : _building->GetAllRooms()) {
        UnivFFviaFM * newfield = new UnivFFviaFM(
            roomPair.second.get(), _building, stepsize, _wallAvoidDistance, _useDistancefield);
    ...
    if(_building->GetConfig()->get_write_VTK_files_direction()) {
        for(unsigned int i = 0; i < _locffviafm.size(); ++i) {
            ...
            iter->second->writeFF(
                "direction" + std::to_string(roomNr) + ".vtk", iter->second->getKnownDoorUIDs());
        }
    }
    ...
    std::chrono::duration<double> elapsed_seconds = end - start;
    Log->Write(
        "INFO: \tTime to construct FF in DirectionLocalFloorfield: " +
...
#if DEBUG
        if(!floorfield->getGrid()->includesPoint(ped->GetPos())) {
            Log->Write("ERROR: \tDirectionSubLocalFloorfield::GetTarget is accessing wrong "
                       "floorfield. Pedestrian is not inside!");
            p = Point(0.0, 0.0);
            return p;
        }
...
void DirectionSubLocalFloorfield::Init(
    Building * buildingArg,
    double stepsize,
    double threshold,
    bool useDistanceMap)
{
    ...
    for(auto & roomPair : _building->GetAllRooms()) {
        for(auto & subPair : roomPair.second->GetAllSubRooms()) {
            ...
            UnivFFviaFM * floorfield = new UnivFFviaFM(
                subPair.second.get(), _building, stepsize, _wallAvoidDistance, _useDistancefield);
...
void DirectionSubLocalFloorfieldTrips::Init(
    Building * buildingArg,
    double stepsize,
    double threshold,
    bool useDistanceMap)
...
void DirectionSubLocalFloorfieldTripsVoronoi::Init(
    Building * buildingArg,
    double stepsize,
    double threshold,
    bool useDistanceMap)
