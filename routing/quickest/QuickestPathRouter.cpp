...
bool QuickestPathRouter::SelectReferencePedestrian(
    Pedestrian * myself,
    Pedestrian ** myref,
    double jamThreshold,
    int exitID,
    ...
    do {
        ...
        if(queue.size() == 0) {
            ...
            for(unsigned int p = 0; p < queue.size(); p++) {
                ...
                if(IsDirectVisibilityBetween(myself, ped) == false)
                    continue;
        ...
        radius = radius * 2;
        if(radius > 100) {
            if(queue.size() > 0) { // there were some ref pedes only not visible
                *myref = nullptr;
                *flag  = UNREACHEABLE_EXIT;
                done   = true;
...
void QuickestPathRouter::GetQueueAtExit(
    Hline * hline,
    double minVel,
    double radius,
    std::vector<Pedestrian *> & queue,
    int subroomToConsider)
{
...
bool QuickestPathRouter::IsDirectVisibilityBetween(Pedestrian * ped, Pedestrian * ref)
{
    ...
    unsigned int obstacles = GetObstaclesCountBetween(
        ped->GetPos(), ref->GetPos(), ignore_hline, ignore_ped1, ignore_ped2);
...
bool QuickestPathRouter::IsDirectVisibilityBetween(Pedestrian * myself, Hline * hline)
{
    ...
    unsigned int obstacles = GetObstaclesCountBetween(
        myself->GetPos(), hline->GetCentre(), hline, ignore_ped1, ignore_ped2);

    return obstacles > _visibilityObstruction ? false : true;
}

unsigned int QuickestPathRouter::GetObstaclesCountBetween(
    const Point & p1,
    const Point & p2,
    Hline * hline,
    int ignore_ped1,
    int ignore_ped2)
{
    ...
    if(sbr1) {
        ...
        for(const auto & ped : peds) {
            //avoiding myself
            if(ped->GetID() == ignore_ped1)
                continue;
            if(ped->GetID() == ignore_ped2)
                continue;
...
int QuickestPathRouter::isCongested(Pedestrian * ped)
{
    ...
    if(allPeds.size() <= _visibilityObstruction)
        return false;
    ...
    for(const auto & ped2 : allPeds) {
        ...
        if(ped2->GetExitIndex() != ped->GetExitIndex())
            continue;
        // skip myself
        if(ped2->GetID() == ped->GetID())
            continue;
    ...
    for(unsigned int g = 0; g < relevantAPs.size(); g++) {
        ...
        if(_defaultStrategy == GLOBAL_SHORTEST) {
            ...
            if((dist > minDistGlobal) //is longer but ...
               && (((dist - minDistGlobal) / (dist + minDistGlobal)) <
                   _cbaThreshold)) //not more than 2 * _cbaThreshold
            {
    ...
    for(TiXmlElement * e = xRouters->FirstChildElement("router"); e;
        ...
        if((strategy == "quickest") && e->FirstChild("parameters")) {
            ...
            if(para) {
                ...
                _queueVelocityFromJam =
                    xmltof(para->Attribute("queue_vel_escaping_jam"), _queueVelocityFromJam);
                _queueVelocityNewRoom =
                    xmltof(para->Attribute("queue_vel_new_room"), _queueVelocityNewRoom);
                _visibilityObstruction = (unsigned int) xmltoi(
                    para->Attribute("visibility_obstruction"), _visibilityObstruction);

                std::string selection_mode =
                    xmltoa(para->Attribute("reference_peds_selection"), "single");
                if(selection_mode == "single")
                    _refPedSelectionMode = RefSelectionMode::SINGLE;
                if(selection_mode == "all")
                    _refPedSelectionMode = RefSelectionMode::ALL;

                std::string default_strategy =
                    xmltoa(para->Attribute("default_strategy"), "local_shortest");
                if(default_strategy == "local_shortest")
                    _defaultStrategy = DefaultStrategy::LOCAL_SHORTEST;
                if(default_strategy == "global_shortest")
                    _defaultStrategy = DefaultStrategy::GLOBAL_SHORTEST;
            }
        }
    }
    return true;
