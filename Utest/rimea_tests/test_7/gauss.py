...
mus = 0.5*(weidman_max_vel+weidman_min_vel)   #[1.095, 1.475, 1.045, 0.61]
sigmas = [0.2, 0.02, 0.13, 0.05] #Parameters mus and sigmas are used in the master-inifile

j = 1
for (mean, sigma, mi, ma) in zip(mus, sigmas, weidman_min_vel, weidman_max_vel):
    plt.subplot("41%d"%j)
    x = np.linspace(mi, ma, 100)
    plt.plot(x, mlab.normpdf(x, mean, sigma), lw=2)
