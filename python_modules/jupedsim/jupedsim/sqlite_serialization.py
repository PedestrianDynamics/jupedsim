...
class SqliteTrajectoryWriter(TrajectoryWriter):
        ...
        *,
        ...
        Args:
            output_file : pathlib.Path
                ...
                Note: the file will not be written until the first call
                to :func:`begin_writing`
            every_nth_frame: int
                indicates interval between writes, 1 means every frame,
                5 every 5th
            commit_every_nth_write: int
                number of frames to keep in RAM before writing them to
                disk.
        """
    ...
    def _trajectory_columns(self) -> str:
        """Return the CREATE TABLE column definitions for trajectory_data."""
        return (
    ...
    def _frame_row(self, frame, agent):
        """Return a tuple of values for one agent in one frame."""
        return (
            frame,
            agent.id,
            agent.position[0],
    ...
    def _insert_placeholders(self) -> str:
        """Return SQL placeholders matching _frame_row length."""
        return "?, ?, ?, ?, ?, ?"

    def begin_writing(self, simulation: Simulation) -> None:
        """Begin writing trajectory data.

        This method is intended to handle all data writing that has to be
        done once before the trajectory data can be written. E.g. Meta
        information such as framerate etc...
        """
    ...
    def write_iteration_state(self, simulation: Simulation) -> None:
        ...
        This method is intended to handle serialization of the trajectory
        data of a single iteration. The default behaviour is to buffer
        frames in memory and only writing to disk when the buffer is full
        or close() is called.
        """
        ...
        try:
            frame_data = [
                self._frame_row(frame, agent) for agent in simulation.agents()
            ]
            cur.executemany(
...
class Sqlite2LvlPedTrajectoryWriter(SqliteTrajectoryWriter):
    ...
    ground support velocity, height, and radius columns.
    """

    def _trajectory_columns(self) -> str:
        return (
            super()._trajectory_columns() + ","
    ...
    def _frame_row(self, frame, agent):
        base = super()._frame_row(frame, agent)
        return base + (
...
def convert_database_v1_to_v2(connection: sqlite3.Connection):
    ...
    try:
        ...
        if version != 1:
            raise RuntimeError(
                f"Internal Error: When converting from database version "
                f"1 to 2, encountered database version {version}."
            )
