...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.0.1/Envelope_3/include/CGAL/Envelope_3/Envelope_element_visitor_3.h $
// $Id: include/CGAL/Envelope_3/Envelope_element_visitor_3.h 50cfbde3b84 $
...
protected:
  ...
  using Self = Envelope_element_visitor_3<Traits, Md2>;
  ...
  using Inner_ccb_iterator = typename Md2::Inner_ccb_iterator;
  using Outer_ccb_iterator = typename Md2::Outer_ccb_iterator;
  using Isolated_vertex_iterator = typename Md2::Isolated_vertex_iterator;
  using Topology_traits = typename Md2::Topology_traits;
  ...
  using Vertices_map = Unique_hash_map<Vertex_handle, Vertex_handle>;
  using Halfedges_map = Unique_hash_map<Halfedge_handle, Halfedge_handle>;
  using Faces_map = Unique_hash_map<Face_handle, Face_handle>;
  ...
  using Intersections_list =
    std::list<std::variant<Intersection_curve, Point_2>>;
  ...
  public:
    ...
    bool operator()(const Point_2_with_info& p1, const Point_2_with_info& p2)
      const {
      ...
      return p_traits->compare_xy_2_object()(p1.first, p2.first) == SMALLER;
    }
  };
...
public:
  ...
  // virtual destructor.
  virtual ~Envelope_element_visitor_3() { if (own_traits) delete m_traits; }
  ...
  void resolve(Face_handle face, Minimization_diagram_2& result) {
    ...
    for (auto inter_objs_it = inter_objs.begin();
      ...
      else CGAL_error_msg( "wrong projected intersection type");
    }
    ...
    for (auto new_edge_it = result_new_edges.begin();
      ...
      if (! f1->is_decision_set()) {
        ...
        else res = resolve_minimal_face(f1, &new_he);
#else
        res = resolve_minimal_face(f1, &new_he);
#endif
        copy_data_by_comparison_result(face, f1, res);
    ...
    for (auto special_vertex_it = result_special_vertices.begin();
         special_vertex_it != result_special_vertices.end();
         ++special_vertex_it) {
  ...
  void resolve(Halfedge_handle edge, Minimization_diagram_2& result) {
      ...
      if (const Point_2* point = std::get_if<Point_2>(&(*inter_objs_it))) {
        ...
        for (auto inter_it = intersections_list.begin();
          ...
          if (const auto* ip = std::get_if<Intersect_point_2>(&(*inter_it)))
            ...
            const auto* icv = std::get_if<X_monotone_curve_2>(&(*inter_it));
    ...
    for (unsigned int i = 0; i < split_points.size(); ++i) {
      ...
      if ((! cur_src_vertex->is_at_open_boundary() &&
        ...
        if (overlaps > 0) finished_part_res = EQUAL;
        else finished_part_res = resolve_minimal_edge(edge, finished_part);

        finished_part->set_decision(finished_part_res);
        finished_part->twin()->set_decision(finished_part_res);

        if (finished_part == edge) first_part_res = finished_part_res;
      }

      // check the overlaps indications
      // we start a new overlapping segment at this point
      if (cur_p.second == true) ++overlaps;
      // we end an overlapping segment at this point
      if (cur_p.third == true) --overlaps;
    }
...
protected:
  ...
  Vertex_handle insert_point(Minimization_diagram_2& arr, const Point_2& p,
    ...
    if ((fh = std::get_if<Face_const_handle>(&obj)) != nullptr) {
