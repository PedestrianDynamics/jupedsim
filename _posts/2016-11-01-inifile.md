...
- `max_peds`: the max number of pedestrians allowed inside the waiting area. **Important:** to avoid unbehaviour `max_peds` should not exceed the number of pedestrians heading for the particulat waiting area. Hence `max_peds(WA1)` <= `max_peds(WA2)`.
...
- `global_timer`: If `true` timer starts with start of the simulation, else timer start when `min_peds` pedestrians are inside waiting area
